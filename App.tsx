        ...
        <div className="fixed inset-0 z-[60] flex justify-end" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-slide-left">
            <div className="p-8 border-b border-stone-100 flex justify-between items-center bg-stone-50">
              <h2 className="text-2xl font-serif font-bold">Tu Lista</h2>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-4">
              {shoppingList.map(item => (
                <div key={item.id} className="flex items-center gap-4 p-4 bg-stone-50 rounded-xl">
                   <button onClick={() => setShoppingList(prev => prev.map(i => i.id === item.id ? {...i, completed: !i.completed} : i))} className={`w-6 h-6 rounded border-2 flex items-center justify-center ${item.completed ? 'bg-amber-600 border-amber-600' : 'border-stone-300'}`}>
                      {item.completed && <span className="text-white text-xs">✓</span>}
                   </button>
                   <span className={`flex-1 ${item.completed ? 'line-through text-stone-400' : ''}`}>{item.name}</span>
                   <button onClick={() => setShoppingList(prev => prev.filter(i => i.id !== item.id))} className="text-stone-300 hover:text-red-500">🗑</button>
                </div>
              ))}
            </div>
          </div>
      ...
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-md border-b border-stone-200 shadow-sm">
        <nav className="max-w-7xl mx-auto px-4 h-20 flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-serif font-bold text-stone-800">Gourmet<span className="text-amber-600">Voice</span></h1>
          </div>
          ...
          <div className="flex items-center gap-2">
            <button onClick={handleRandomRecipe} className="p-2.5 rounded-full hover:bg-amber-50 text-amber-600 transition-colors" title="Inspiración aleatoria">🎲</button>
            <button onClick={() => setIsHighContrast(!isHighContrast)} className={`p-2.5 rounded-full transition-all ${isHighContrast ? 'bg-stone-900 text-white' : 'hover:bg-stone-100 text-stone-400'}`}>🌓</button>
            <button onClick={() => setShowFavoritesOnly(!showFavoritesOnly)} className={`p-2.5 rounded-full transition-all ${showFavoritesOnly ? 'bg-amber-100 text-amber-600' : 'hover:bg-stone-100 text-stone-400'}`}>⭐</button>
      ...
      <main className="max-w-7xl mx-auto px-4 py-10">
          ...
          {filteredRecipes.map(recipe => (
            ...
            >
              ...
              <div className="p-6">
                ...
                <h3 className="text-lg font-serif font-bold text-stone-800 leading-tight mb-4">{recipe.title}</h3>
                <div className="flex items-center justify-between text-[10px] font-bold text-stone-400">
                  ...
                  <span className="flex items-center gap-1">🍽 {recipe.ingredients.length} ing.</span>
                </div>
              </div>
            </article>
          ))}
        ...
        {filteredRecipes.length === 0 && (
          <div className="text-center py-40">
            ...
            <p className="text-stone-400 font-serif italic text-xl">No encontramos recetas con esos criterios...</p>
          </div>
        )}
      </main>
