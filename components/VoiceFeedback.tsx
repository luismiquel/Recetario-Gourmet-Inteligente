...
export const VoiceFeedback: React.FC<VoiceFeedbackProps> = ({ status }) => {
  ...
  const config = {
    ...
    speaking: {
      bg: 'bg-amber-500',
      label: 'Asistente hablando',
      subtext: 'Escucha con atención',
      icon: (
    ...
    error: {
      bg: 'bg-stone-800',
      label: 'Sin señal',
      subtext: 'Repite el comando por favor',
      icon: (
    ...
    <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-[100] flex flex-col items-center pointer-events-none w-full max-w-xs sm:max-w-md" aria-live="assertive">
      <div className={`${config.bg} ${config.animate} flex items-center gap-4 px-6 py-4 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] transition-all duration-500 transform border border-white/20 backdrop-blur-lg`}>
        ...
        <div className="flex flex-col items-start leading-tight">
          <span className={`text-[10px] font-black uppercase tracking-[0.2em] ${status === 'processing' ? 'text-stone-900' : status === 'speaking' ? 'text-stone-950' : 'text-white'}`}>
            {config.label}
          </span>
